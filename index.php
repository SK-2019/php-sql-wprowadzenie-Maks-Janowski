...
$sql=('SELECT * from pracownicy,organizacja where dzial=id_org  order by imie asc');
    $result=$conn->query($sql); //mysql
        ...
        echo("</table>");
echo("<hr />");
    $sql=('SELECT dzial, max(zarobki) AS Maksimum FROM pracownicy GROUP BY dzial');
    $result=$conn->query($sql); //mysql
        echo("<h3>Max zarobkow</h3>");//nazwa nad tabelą
        echo("<li>$sql");
        echo("<table border=1>");
        echo("<th>id</th>");
        echo("<th>imie</th>");
        ...
        echo("</table>");
echo("<hr />");
    $sql=('SELECT Dzial, sum(zarobki) AS Całość, avg(zarobki) AS Średnia FROM pracownicy GROUP BY dzial');
    $result=$conn->query($sql); //mysql
        echo("<h3>Suma zarobkow</h3>");//nazwa nad tabelą
        echo("<li>$sql");
        echo("<table border=1>");
        echo("<th>id</th>");
        echo("<th>imie</th>");
